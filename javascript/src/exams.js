...
// Filtra meses
const filteredMonths = months.filter( (months)=> { return months.days === 31 });
...
// Lista meses
filteredMonths.forEach(month => { toPrint += month.month + ', ' });
...
let years = cachorros.map( (cachorro => { return cachorro * 7 }));

document.getElementById('main').innerHTML = 
` ${toPrint} <br> Soma dos dias dos meses selecionados: ${sumMonthDays.days} <br> idade humana dos cachorros: ${years}`;
